          ...
          for data, index in this.load_files
            ...
            axios
              ...
              .then (response) =>
                  ...
                  uri = response.headers['content-disposition']
                      .split("filename*=utf-8''")[1]
                  if uri == undefined
                      uri = response.headers['content-disposition']
                      .split("filename=")[1].replace('\"', '').replace('\"', '')
                  console.log(uri)
                  this.image_items.push.apply(this.image_items, [uri])
                  # this.image_items.push.apply(this.image_items, [testuri])
                  this.load_files.length=0 # 簡易的なリセット　複数ファイルのときはダメ
                  console.log(this.image_items)
              .catch (response) =>
